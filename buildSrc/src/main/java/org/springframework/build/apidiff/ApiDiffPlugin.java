...
package org.springframework.build.apidiff;
...
/**
 * {@link Plugin} that applies an API diff plugin (here, {@code "japicmp-gradle-plugin"})
 * and create tasks for all subprojects, diffing the public API one by one
 * and creating the reports in {@code "build/reports/apidiff/$OLDVERSION_to_$NEWVERSION/"}.
 * <p>{@code "./gradlew apiDiff -PbaselineVersion=5.1.0.RELEASE"} will output the
 * reports for the API diff between the baseline version and the current project.
 *
 * @author Brian Clozel
 */
public class ApiDiffPlugin implements Plugin<Project> {

	private static final String TASK_NAME = "apiDiff";
	...
	public void apply(Project project) {
		if (project.hasProperty(BASELINE_VERSION_PROPERTY)
				&& project.getRootProject().equals(project)) {
	...
	private void createApiDiffTask(String baselineVersion, Project project) {
		if (isProjectEligible(project)) {
			...
			apiDiff.setIncludeSynthetic(true);
