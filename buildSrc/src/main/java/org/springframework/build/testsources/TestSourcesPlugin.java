...
public class TestSourcesPlugin implements Plugin<Project> {
	...
	private static final List<String> CONFIGURATIONS = Arrays.asList("compile", "optional", "testCompile");

	@Override
	public void apply(Project project) {
		project.getPlugins().withType(JavaLibraryPlugin.class, (plugin) -> addTestSourcesToProject(project));
	}
	...
	private void addTestSourcesFromDependency(final Project currentProject, ProjectDependency dependency) {
		...
		dependencyProject.getPlugins().withType(JavaPlugin.class, plugin -> {
			...
			SourceSetOutput test = javaPlugin.getSourceSets().findByName("test").getOutput();
			currentProject.getDependencies().add("testCompile", test);
		});
	}
}
