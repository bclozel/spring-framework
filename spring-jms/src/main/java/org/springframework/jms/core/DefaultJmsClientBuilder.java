...
class DefaultJmsClientBuilder implements JmsClient.Builder {
	...
	public JmsClient build() {
		if (this.messageConverter != null) {
			this.jmsClient.setMessageConverter(this.messageConverter);
		}
		if (this.messagePostProcessors != null) {
			this.jmsClient.setMessagePostProcessor(new DelegatingMessagePostProcessor(this.messagePostProcessors));
		}
		return this.jmsClient;
	}

	static class DelegatingMessagePostProcessor implements MessagePostProcessor {

		private final List<MessagePostProcessor> messagePostProcessors;

		public DelegatingMessagePostProcessor(List<MessagePostProcessor> messagePostProcessors) {
